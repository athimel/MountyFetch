...
public class PublicDataProvider {
    ...
    protected static final Cache<String, String> TROLLS_BY_NAME = CacheBuilder.newBuilder().expireAfterWrite(2, TimeUnit.MINUTES).build();
    ...
    public Optional<String> readTrollsLine(String trollName) {
        String line = TROLLS_BY_NAME.getIfPresent(trollName);
        if (StringUtils.isEmpty(line)) {
            try {
                List<String> lines = fetchPublicFile("Public_Trolls.txt");
                lines.forEach(l -> {
                    ...
                    if (matcher.matches()) {
                        String name = matcher.group(2);
                        TROLLS_BY_NAME.put(name, l);
                    } else {
                        System.err.println("Ligne 'Trolls' invalide: " + l);
                    }
                });

                line = TROLLS_BY_NAME.getIfPresent(trollName);
            } catch (IOException ioe) {
                throw new UncheckedIOException(ioe);
            }
        }
